...
export function wrapSexpr(doc, open, close, start = doc.selectionStart, end = doc.selectionEnd) {
...
export function joinSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    cursor.backwardWhitespace();
...
export function spliceSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
...
export function killBackwardList(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    // NOTE: this should unwrap the string, not throw.
    if (cursor.withinString())
...
export function killForwardList(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
...
export function spliceSexpKillingBackward(doc, start = doc.selectionEnd) {
    spliceSexp(doc, killBackwardList(doc, start));
}
export function spliceSexpKillingForward(doc, start = doc.selectionEnd) {
    spliceSexp(doc, killForwardList(doc, start));
}
export function forwardSlurpSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    cursor.forwardList();
...
export function backwardSlurpSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    cursor.backwardList();
...
export function forwardBarfSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    cursor.forwardList();
    if (cursor.getToken().type == "close") {
        let offset = cursor.offsetStart;
...
export function backwardBarfSexp(doc, start = doc.selectionEnd) {
    let cursor = doc.getTokenCursor(start);
    cursor.backwardList();
    let tk = cursor.getPrevToken();
    if (tk.type == "open") {
...
export function open(doc, pair, start = doc.selectionEnd) {
    doc.insertString(pair);
    doc.selectionStart = doc.selectionEnd = start + 1;
}
export function close(doc, close, start = doc.selectionEnd) {
...
export function backspace(doc, start = doc.selectionStart, end = doc.selectionEnd) {
    if (start != end) {
        doc.backspace();
    }
    else {
...
export function deleteForward(doc, start = doc.selectionStart, end = doc.selectionEnd) {
    if (start != end) {
        doc.delete();
    }
    else {
...
export function stringQuote(doc, start = doc.selectionStart, end = doc.selectionEnd) {
    if (start != end) {
        doc.insertString('"');
    }
    else {
...
export function growSelection(doc, start = doc.selectionStart, end = doc.selectionEnd) {
...
export function shrinkSelection(doc) {
...
export function raiseSexp(doc, start = doc.selectionStart, end = doc.selectionEnd) {
    if (start == end) {
...
export function convolute(doc, start = doc.selectionStart, end = doc.selectionEnd) {
    if (start == end) {
